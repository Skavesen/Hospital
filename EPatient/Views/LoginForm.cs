    ...
    public partial class LoginForm : MetroFramework.Forms.MetroForm
    {
        ...
        private void buttonLogin_Click(object sender, EventArgs e)
        {
            if (ValidateChildren(ValidationConstraints.Enabled))
            {
                if (_helper.LoggedIn(textUsername.Text, textPassword.Text, checkBoxRememberMe.Checked))
                {
                    ...
                    MetroFramework.MetroMessageBox.Show(this, "Неправильные учетные данные!", "Error", MessageBoxButtons.OK,
        ...
        private void textUsername_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(textUsername.Text))
            {
                e.Cancel = true;
                textUsername.Focus();
                loginErrorProvider.SetError(textUsername, "Пожалуйста, введите имя пользователя");
            }
            else
            {
                e.Cancel = false;
