...
namespace EPatient.Views.Admin
{
    public partial class ServicesControl : MetroFramework.Controls.MetroUserControl
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateChildren(ValidationConstraints.Enabled))
                {
                    ...
                    MetroFramework.MetroMessageBox.Show(this, "Успех", "Message", MessageBoxButtons.OK,
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (MetroFramework.MetroMessageBox.Show(this, "Удалить запись?", "Message",
        ...
        private void textName_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(textName.Text) || textName.Text.Length > 255)
            {
                e.Cancel = true;
                textName.Focus();
                errorProvider.SetError(textName, "Пожалуйста, введите название службы (максимум 255 символов)");
            }
            else
            {
                e.Cancel = false;
        ...
        private void textPrice_Validating(object sender, CancelEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                e.Cancel = true;
                textPrice.Focus();
                errorProvider.SetError(textPrice, "Пожалуйста, введите десятичное число");
                return;
            }
        ...
        private void textDuration_Validating(object sender, CancelEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                e.Cancel = true;
                textPrice.Focus();
                errorProvider.SetError(textDuration, "Пожалуйста, введите натуральное число");
                return;
            }
        ...
        private void cboPavilion_Validating(object sender, CancelEventArgs e)
        {
            if (cboPavilion.SelectedIndex < 0)
            {
                e.Cancel = true;
                cboPavilion.Focus();
                errorProvider.SetError(cboPavilion, "Пожалуйста, выберите отделение");
            }
            else
            {
                e.Cancel = false;
